...
export async function GET(request: NextRequest) {
  try {
    ...
    if (slug && mapped.length > 0) {
      ...
      try {
        // Buscar docentes vinculados a este curso (courseId ahora es TEXT) en la escuela correcta
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      revalidatePath(`/${schoolId}`)
      revalidatePath(`/${schoolId}/formaciones`)
    } catch (_) {}
